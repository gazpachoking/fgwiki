...
||aria_config||N/A||Yes||This is the "parent folder" of any parameters that should get passed to aria2. Any [[command line option] available in aria2 can be used by removing the two dashes (--) in front of the command name and changing ```key=value``` to ```key: value```. Any parameter can contain the name of any [http://flexget.com/wiki/Entry entry field] using [http://flexget.com/wiki/Plugins/set#[[Jinja2Templating]] jinja2 notation|http://aria2.sourceforge.net/manual/en/html/aria2c.html#options]] and will be rendered prior to passing to aria2. See sample configuration below.||
* Default for ```file_exts```: [['.avi', '.mp4', '.wmv', '.asf', '.divx', '.mov', '.mpg', '.rm'|'.mkv',]]
...
We also want to rename the content files and provide a renaming template. Finally we are passing several commands to aria2 to affect how it processes the files, the most important of which is ```dir``` since it tells aria2 where to store the files. Note the use of [[jinja2 notation] to dynamically insert the  ```{{series_name}}```, per download, prior to passing to aria2. Any field in the [http://flexget.com/wiki/Entry entry|http://flexget.com/wiki/Plugins/set#[[Jinja2Templating]]]] can be included in the same manner.
